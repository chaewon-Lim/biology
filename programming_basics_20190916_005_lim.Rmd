...
The formula for the sum of the series What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the
...
In each iteration n = 1, n = 2, etc…, we compute S and store it in the nth entry of `s_n`.
...
8. Define an empty numerical vector `s_n` of size 25 using `s_n <- vector("numeric", 25)` and store in the results of    using a for-loop.
...
9. peat exercise 8, but this time use `sapply`.
...
11. Plot  S versus n. Use points defined by n=1,…,25.
