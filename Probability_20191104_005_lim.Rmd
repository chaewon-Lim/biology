...
We use the  ∣  as shorthand for “given that” or “conditionalon”.
...
The multiplication rule also applies to more than two events. We can use induction to expand for more events: Pr(A and B and C)=Pr(A)Pr(B|A)Pr(C∣A and B)
...
But to multiply like this we need to assume independence! Say the conditional probability of a man having a mustache conditional on him having a beard is .95. So the correct calculation probability is much higher:  1/10×95100=0.095 .
...
```{r}
balls <- paste(beads, c(1:3, 1:5, 1:7))
hand <- permutations(15, 2, v = balls)
first_hand <- hands[,1]
second_hand <- hands[,2]
...
```{r}
mean(beads %in% "cyan") * mean(!beads %in% "cyan")
```
...
B <- 10000
games <- replicate(B, {
  win <- sample(c(0,1), 4, replace = TRUE, prob = c(0.6, 0.4))
  any(win==1)
})
mean(games)
```
